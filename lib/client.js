...
Client.prototype._initializeOptions = function(options) {
  ...
  this.wsdl.options.fixLocation = options.fixLocation;
};
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  if (this.wsdl.options.fixLocation)
    location = this.wsdl.options.fixLocation(location);
  ...
  xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
    ...
    encoding +
    (this.wsdl.options.extraSoapHeaders || '') + " " +
    ...
    req.on('response', function (response) {
      ...
      if(response.statusCode !== 200 || !output || !output.$lookupTypes) {
        response.pipe(concatStream({encoding: 'string'}, function (body) {
          ...
          let xmlStartPosition = body.indexOf('\r\n\r\n');
          if (xmlStartPosition !== -1)
            body = body.substring(xmlStartPosition + 4, body.lastIndexOf('>') + 1);
